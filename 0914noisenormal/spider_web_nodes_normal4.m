function d = spider_web_nodes_normal4(n,a) %n is the number of the edge 
...
t = [];
for i = 0:n-1;
    t = [t;i*2*pi/n+rand_in_range([-0.2 0.2],1)];
end
for a = 1:a;
       for i = 0:n-1;
        %t=i*2*pi/n+rand_in_range([0.1 0.2],1);    %theta=(n-2)*pi/2/n;r=c/2/cos(theta);
        t1 = t(i+1,:);
        r1 = r; %+ rand_in_range([-0.4 0.4],1);
        d.x_pos = [d.x_pos; r1*cos(t1)];
        d.y_pos = [d.y_pos; r1*sin(t1)];
    end
    r=r+r*rand_in_range([0.3 0.8],1);
end
d.w_idx=0;
