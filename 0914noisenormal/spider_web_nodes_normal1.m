function d = spider_web_nodes_normal1(n,a,delta) %n is the number of the edge 
                                   %a is the number of level;circle
%  zhibian meige jiaodu de r, jiaodu dengfen
r = 1.2143;
d.x_pos = [];
d.y_pos = [];
d.num = 1 + a*n;
...
d.y_pos = [d.y_pos; 0];
for a = 1:a;
    for t=(0:n-1)*360/n;%+rand_in_range([0.1 0.2],1);    %theta=(n-2)*pi/2/n;r=c/2/cos(theta);
        r1 = normrnd(r,delta);
        d.x_pos = [d.x_pos; r1*cosd(t)];
        d.y_pos = [d.y_pos; r1*sind(t)];
    end
    r=r+2;%r*rand_in_range([0.1 1],1);
end
d.w_idx=0;
