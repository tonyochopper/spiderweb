...
%  
% Hauser, H.; Ijspeert, A.; F¨¹hslin, R.; Pfeifer, R. & Maass, W.
...
net.init_data = d; 
% f = spider_web_nodes_d(8,4);
% [x_pos,y_pos] = spider_web_nodes(8,4)
%  d.num = 8*4+1;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
net.P.states(:,1) = d.x_pos;
net.P.states(:,2) = d.y_pos;
...
%fix all nodes in the outside circle
fixed_idx = [d.num-d.n+1:d.num]'
for i = 1:size(fixed_idx,1)
    net.P.fixed(fixed_idx(i,1),1)=1;
end
...
R = zeros(d.num,d.num);
 for i=1:size(d.from)
   from = d.from(i,1);to = d.to(i,1);  % 1 --> 2
   R(from,to)=1;
...
% net.W.from = d.from;	  % index of from point
% net.W.to   = d.to;    % index of from point
